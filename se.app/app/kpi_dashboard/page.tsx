...
import { ChangeOrdersChart } from "@/components/dashboard/charts/ChangeOrdersChart";
import { BusinessMixChart } from "@/components/dashboard/charts/BusinessMixChart";
import { LabourEfficiencyChart } from "@/components/dashboard/charts/LabourEfficiencyChart";
import { ProjectsTable } from "@/components/dashboard/ProjectsTable";
import { ProjectDetailModal } from "@/components/dashboard/ProjectDetailModal";
import { AdvancedAnalytics } from "@/components/dashboard/AdvancedAnalytics";
import { TeamLeadersAnalytics } from "@/components/dashboard/TeamLeadersAnalytics";
...
const Index = () => {
  ...
  const kpis = useMemo(() => {
    ...
    const netVariance = filteredProjects.reduce((sum, p) => sum + p.profit.netVariance, 0);
    ...
    return {
      ...
      projectedVsActual: {
        ...
        actual: actualHours,
        delta: ((actualHours - projectedHours) / projectedHours) * 100,
        sparkline: [
          projectedHours * 0.9,
          projectedHours * 0.95,
          projectedHours,
  ...
  // Chart data - Generate from filtered projects
  ...
  const leaderMetrics = useMemo(() => {
    ...
    return Array.from(leaderMap.values()).map(leader => ({
      ...
      efficiency: leader.totalProjectedHours > 0 ? (leader.totalProjectedHours / leader.totalActualHours) * 100 : 0,
          ...
          <TabsContent value="overview" className="space-y-8 animate-fade-in">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
              ...
              <KpiCard
                ...
                subtitle={`${((kpis.grossProfit.value / kpis.totalRevenue.value) * 100).toFixed(1)}% margin`}
              ...
              <KpiCard
                ...
                subtitle={`${((kpis.netProfit.value / kpis.totalRevenue.value) * 100).toFixed(1)}% margin`}
